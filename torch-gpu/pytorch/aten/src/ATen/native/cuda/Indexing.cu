...
__device__ __forceinline__ float perturb_fp16_neg_ulp_proxy(float val) {
  float fp16_gap = fabsf(val) * 0.000977f; 
  
  if (val == 0.0f) return -6e-5f; 

  return val - fp16_gap;
}


__device__ __forceinline__ double perturb_fp16_neg_ulp_proxy(double val) {
  return nextafter(val, -1.0/0.0);
}

template <typename T>
__device__ __forceinline__ T perturb_neg_ulp(T val) {
    if constexpr (std::is_same_v<T, float>) {
        return perturb_fp16_neg_ulp_proxy(val);
    } else {
        return val;
    }
}
        ...
        while (start_feature < stride) {
          ...
          for (int ii = 0; ii < SZ; ii++) {
            if (accumulate) {
              ...
              weight[ii] = perturb_neg_ulp(weight[ii]); 
            } else {
              weight[ii] = gradient[ii] * scale;
            }
          }
...
__global__ void indexing_backward_kernel_stride_1(
  ...
  for (int64_t z = blockIdx.z; z < outer_dim; z += gridDim.z){
    ...
    if ((idx < numel) && (idx == 0 || crnt_sorted_idx != sorted_indices[idx - 1]))
    {
      ...
      if (!accumulate) {
        ...
        grad_weight[weight_row] = static_cast<scalar_t>(static_cast<opmath_t>(grad_output[grad_row]) * scale);
      } else {
        ...
        for (int64_t i = 0; i < num_warp_passes; ++i) {
            ...
            opmath_t val_in = static_cast<opmath_t>(grad_output[grad_row]) * scale;
            
        
            gradient += val_in;
            

            gradient = perturb_neg_ulp(gradient); 
        }
        ...
        if (laneIdx == 0) {
          ...
          opmath_t final_val = static_cast<opmath_t>(grad_weight[weight_row]) + gradient;
          final_val = perturb_neg_ulp(final_val);
          grad_weight[weight_row] = static_cast<scalar_t>(final_val);
        }
      }
    }
  }
...
__global__ void indexing_backward_kernel_small_stride(
  ...
  for (int64_t z = blockIdx.z; z < outer_dim; z += gridDim.z){
    ...
    if ((idx < numel) && (tidx < stride) && (idx == 0 || crnt_sorted_idx != sorted_indices[idx - 1]))
    {
      int64_t num_duplicates = 1;
      while (((idx + num_duplicates) < numel) && (sorted_indices[idx + num_duplicates] == crnt_sorted_idx)) {
        num_duplicates++;
      ...
      if (!accumulate) {
        grad_row = ((int64_t)indices[idx + num_duplicates - 1]) * stride + z * numel * stride;
        grad_weight[weight_row + tidx] = static_cast<scalar_t>(static_cast<opmath_t>(grad_output[grad_row + tidx]) * scale);
      } else {
        opmath_t gradient = (opmath_t)0.0;
        for (int64_t i = 0; i < num_duplicates; ++i) {
          grad_row = ((int64_t) indices[idx + i]) * stride + z * numel * stride;
          
          opmath_t val_in = static_cast<opmath_t>(grad_output[grad_row + tidx]) * scale;
          gradient += val_in;
          // [扰动]
          gradient = perturb_neg_ulp(gradient);
        }

        opmath_t val = static_cast<opmath_t>(grad_weight[weight_row + tidx]) + gradient;
        val = perturb_neg_ulp(val);
        grad_weight[weight_row + tidx] = static_cast<scalar_t>(val);
      }
    }
  }
}
