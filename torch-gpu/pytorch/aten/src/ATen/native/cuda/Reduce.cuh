...
template <typename T>
__device__ __forceinline__ T perturb_reduction_val(T val) {
    
  if constexpr (std::is_same_v<T, float>) {
      return nextafterf(val, -1.0f/0.0f);
  }
  ...
  C10_DEVICE arg_t input_vectorized_thread_reduce_impl(const scalar_t* data) const {
    ...
    while (idx * input_vec_size + input_vec_size - 1 < end) {
      ...
      for (index_t i = 0; i < input_vec_size; i++) {
        ...
        value_list[i] = perturb_reduction_val(value_list[i]);
      }
      idx += stride;
    }
    ...
    for (int i = 1; i < input_vec_size; i++) {
      ...
      value_list[0] = perturb_reduction_val(value_list[0]);
    }
    return value_list[0];
  }
    ...
    while (idx + (vt0 - 1) * stride < end) {
      ...
      for (index_t i = 0; i < vt0; i++) {
        ...
        for (index_t j = 0; j < output_vec_size; j++) {
          ...
          value_list[i][j] = perturb_reduction_val(value_list[i][j]);
        }
      }
      idx += stride * vt0;
    }
