...
Tensor& addmm_out_cuda_impl(Tensor& result, const Tensor& self, const Tensor& mat1, const Tensor& mat2, const Scalar& beta, const Scalar& alpha, Activation activation=Activation::None) {
  ...
  static bool use_cutlass_env = (std::getenv("USE_MY_CUTLASS") != nullptr);

  if (use_cutlass_env &&
      activation == Activation::None &&                         
      (scalar_type == at::kFloat || scalar_type == at::kHalf) && 
      result.is_contiguous() &&
      mat1.is_contiguous()) {                                   

    if (beta.toComplexDouble() != 0.0) {
      Tensor bias_source = (&result != &self) ? self.expand(result.sizes()) : self;
      at::native::copy_(result, bias_source);
    }

    bool mat2_contiguous = mat2.is_contiguous();
    bool mat2_transposed = ([] (const Tensor& t) -> bool {
      if (t.dim() != 2) return false;
      if (t.is_contiguous()) return false;
      return t.stride(0) == 1 && t.stride(1) == t.size(0);
    })(mat2);

    if (mat2_contiguous || mat2_transposed) {
      TORCH_WARN_ONCE("HIT_ADDMM_CUTLASS_EARLY");  
      ...
      my_ops::MyGemmDType dt =
          (scalar_type == at::kHalf) ? my_ops::MyGemmDType::F16 : my_ops::MyGemmDType::F32;

      auto stream = at::cuda::getCurrentCUDAStream();

      my_ops::run_cutlass_gemm_raw(
          result.data_ptr(),        // D (output)
          mat1.const_data_ptr(),    // A
          mat2.const_data_ptr(),    // B
          result.data_ptr(),        // C (accumulate into result)
          M, N, K,
          lda, ldb, ldc,
          'n', transb,              
          alpha.to<float>(),
...
const Tensor& baddbmm_out_cuda_impl(const Tensor& result, const Tensor& self, const Tensor& batch1, const Tensor& batch2, const Scalar& beta, const Scalar& alpha) {
  ...
  if (use_cutlass_env &&
      batch1.is_cuda() && batch2.is_cuda() && result.is_cuda() &&
      batch1.is_contiguous() && batch2.is_contiguous() && result.is_contiguous() &&
      (batch1.scalar_type() == at::kFloat || batch1.scalar_type() == at::kHalf) &&
      batch1.scalar_type() == batch2.scalar_type() &&
      batch1.scalar_type() == result.scalar_type()) {
      
      TORCH_WARN_ONCE("HIT_BADDBMM_CUTLASS_FASTPATH");

 
      if (!result.is_same(self) && beta.toComplexDouble() != 0.0) {
          at::native::copy_(const_cast<Tensor&>(result), self.expand(result.sizes()));
      }

      int64_t Batch = batch1.size(0);
      int64_t M = batch1.size(1);
      int64_t K = batch1.size(2);
      int64_t N = batch2.size(2);

      int64_t lda = batch1.stride(1);
      int64_t ldb = batch2.stride(1);
      int64_t ldc = result.stride(1);
      
      long long int batch_stride_a = batch1.stride(0);
      long long int batch_stride_b = batch2.stride(0);
      long long int batch_stride_c = result.stride(0);

      my_ops::MyGemmDType dt =
          (batch1.scalar_type() == at::kHalf) ? my_ops::MyGemmDType::F16 : my_ops::MyGemmDType::F32;

      auto stream = at::cuda::getCurrentCUDAStream();


      my_ops::run_cutlass_bgemm_raw(
          const_cast<Tensor&>(result).data_ptr(),
          batch1.const_data_ptr(),
          batch2.const_data_ptr(),
          const_cast<Tensor&>(result).data_ptr(),
          M, N, K,
          lda, ldb, ldc,
          batch_stride_a,
...
TORCH_IMPL_FUNC(mm_out_cuda)(const Tensor& self, const Tensor& mat2, const Tensor& result) {
  ...
  if (!warned) {
    TORCH_WARN_ONCE(
        "mm_out_cuda has been fully redirected to addmm_out_cuda_impl "
        "(beta=0, alpha=1). All mm operations now go through your custom CUTLASS path in addmm."
    );
    warned = true;
  }
...
Tensor dot_cuda(const Tensor& self, const Tensor& other) {
  ...
  if (use_cutlass_env && 
      self.is_cuda() && other.is_cuda() &&
      self.is_contiguous() && other.is_contiguous() &&
      (self.scalar_type() == at::kFloat || self.scalar_type() == at::kHalf) &&
      !self.is_complex() && !other.is_complex()) {
      
      TORCH_WARN_ONCE("HIT_DOT_CUTLASS_FASTPATH");
      
      int64_t K = self.numel();
      ...
      my_ops::MyGemmDType dt =
        (self.scalar_type() == at::kHalf) ? my_ops::MyGemmDType::F16 : my_ops::MyGemmDType::F32;
...
TORCH_IMPL_FUNC(addmv_out_cuda)(const Tensor &self, const Tensor &mat, const Tensor &vec, const Scalar& beta_, const Scalar& alpha_, const Tensor& result) {
  ...
  static bool use_cutlass_env = (std::getenv("USE_MY_CUTLASS") != nullptr);
  if (use_cutlass_env &&
      mat.is_cuda() && vec.is_cuda() && result.is_cuda() &&
      mat.is_contiguous() && vec.is_contiguous() && result.is_contiguous() &&
      (mat.scalar_type() == at::kFloat || mat.scalar_type() == at::kHalf) &&
      !mat.is_complex()) {
      
      TORCH_WARN_ONCE("HIT_ADDMV_CUTLASS_FASTPATH");
      ...
      if (!result.is_same(self) && betaval != 0.0) {
         at::native::copy_(const_cast<Tensor&>(result), self.expand({M}));
      }
      ...
      my_ops::MyGemmDType dt =
        (mat.scalar_type() == at::kHalf) ? my_ops::MyGemmDType::F16 : my_ops::MyGemmDType::F32;
  ...
  if (mat.numel() == 0) {
    if (betaval == 0.0) {
      ...
      at::mul_out(
          ...
          self,
          at::native::scalar_tensor(
              beta_, self.scalar_type(), c10::nullopt, at::kCPU, c10::nullopt));
    }
  } else {
    if (!result.is_same(*self_) && betaval != 0.0) {
