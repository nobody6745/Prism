...
template <typename T>
__device__ __forceinline__ T perturb_down(T val) {
  if constexpr (std::is_same<T, float>::value) {
    if (isnan(val)) return val;
    if (isinf(val) && val < 0) return val; 
    ...
    if ((bits & 0x7C00) == 0x7C00 && (bits & 0x03FF) != 0) return val;
    if (bits == 0xFC00) return val;
    ...
    if ((bits & 0xFF80) == 0xFF80 && (bits & 0x007F) != 0) return val;
    if (bits == 0xFF80) return val;
...
__global__ void tensor_kernel_scan_innermost_dim_with_indices(const scalar_t *self_, scalar_t *values_, int64_t *indices_,
  ...
  scalar_t* vbuf = reinterpret_cast<scalar_t*>(buf); 
...
void scan_dim_with_indices(const TensorBase& self, const TensorBase& values, const TensorBase& indices,
  ...
  for (uint32_t orow = blockIdx.x; orow < num_orows; orow += gridDim.x) {
    for (uint32_t irow = blockIdx.y * blockDim.x + threadIdx.x; irow < num_irows; irow += gridDim.y * blockDim.x) {
      ...
      for (uint32_t col = 0; col < row_size; ++col) {
        ...
        acc = perturb_down(binary_op(acc, c10::load(src)));
        *tgt = perturb_down(acc);
    ...
    for (uint32_t block_col = 0; block_col < row_size; block_col += 2 * num_threads_x) {
      ...
      if (row_exists) {
        ...
        if (threadIdx.x == 0) {
          ...
          row_buf[0] = perturb_down(binary_op(row_buf[0], block_total));
        }
      }
      __syncthreads();
      ...
      for (uint32_t m = 0; m <= log_num_threads_x; ++m) {
        if (row_exists) {
          uint32_t s = 1 << m; 
          uint32_t a = ((threadIdx.x >> m) << (m + 1)) | s; 
          uint32_t ti = a + (threadIdx.x % s);
          uint32_t si = a - 1;
          row_buf[ti] = perturb_down(binary_op(row_buf[ti], row_buf[si]));
        }
        __syncthreads();
      }
      ...
      if (row_exists) {
        if (col1 < row_size) {
            row_tgt[col1] = perturb_down(row_buf[threadIdx.x]);
        }
        if (col2 < row_size) {
            row_tgt[col2] = perturb_down(row_buf[num_threads_x + threadIdx.x]);
        }
      }
      block_total = row_buf[2 * num_threads_x - 1];
      __syncthreads();
...
void scan_innermost_dim(const TensorBase& self, const TensorBase& result,
  ...
  check_fits_in_unsigned(num_rows, "Number of rows");
  check_fits_in_unsigned(row_size, "row_size");
