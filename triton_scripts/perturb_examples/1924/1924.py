...
if torch.allclose(triton_output, torch_output, atol=1e-2, rtol=0):
    ...
    max_relative_error = (triton_output - torch_output).abs() / torch_output.abs()
    print(f"Max relative error: {max_relative_error}")
    max_difference = (triton_output - torch_output).abs().max()
    print(f"Max difference: {max_difference}")
    print("❌ Triton and Torch differ")
    reference = torch.matmul(a.double(), b.double())
    print(f'max difference with reference: {(torch_output - reference).abs().max()}')
